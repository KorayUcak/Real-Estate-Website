...
import { isRouteLocale, languageFromRoute } from "@/lib/locale";
...
export default async function SiteLayout({ children }: LayoutProps<"/[lang]">) {
  ...
  /*
    Dil rotadan geliyor ve buradan İSTEMCİYE iniyor. `LocaleProvider` artık
    dili localStorage'dan okumuyor — sunucuda çözülmüş değeri prop olarak
    alıyor (gerekçe components/locale-provider.tsx).
  */
  const locale = await langParam();
  const language = languageFromRoute(
    isRouteLocale(locale) ? locale : "en",
  );
      ...
      */}
      ...
      <ConsentProvider>
        <SettingsProvider value={settings}>
          <LocaleProvider language={language} rates={rates}>
            {/*
              Atlama bağlantısı SAĞLAYICILARIN İÇİNDE: `<T>` çeviri bağlamına
              ihtiyaç duyuyor. Konumu görsel olarak değişmiyor — odaklanana
              kadar `sr-only`, odaklanınca sayfanın sol üstünde.
            */}
            <a
              href="#main"
            ...
            {/*
              BANNER'IN DOM'DAKİ YERİ: başlıktan ÖNCE, atlama bağlantısından
              sonra. Ekranda `fixed` ile altta duruyor; buradaki erken konum
              yalnızca SEKME SIRASI için — klavye kullanıcısı sayfaya girer
              girmez rıza seçimine ulaşıyor, otuz gezinme bağlantısını
              geçtikten sonra değil.
            */}
            <CookieBanner />
            ...
            {/*
              Clarity SAĞLAYICININ İÇİNDE ve rızaya bağlı: "Accept all"
              denmeden clarity.ms'e tek bir istek bile gitmiyor
              (bkz. components/clarity.tsx).
            */}
            <Clarity />
          </LocaleProvider>
        </SettingsProvider>
